    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-4 lg:px-6">
        <div class="relative flex items-center justify-between px-2 sm:px-4 py-3 rounded-2xl border bg-slate-900/90 backdrop-blur-xl border-slate-700/50 shadow-2xl">
            <a href="{{ url('/') }}" class="flex items-center gap-2 group">
                ...
                <div class="relative">
                   <span class="font-black text-2xl tracking-tighter uppercase text-slate-100 flex items-baseline">
                        {{ explode(' ', $profile->full_name)[0] ?? '' }}<span class="text-[17px] ml-0.5 transition-colors text-slate-500 group-hover:text-cyan-500">{{ explode(' ', $profile->full_name)[1] ?? '' }}</span>
                   </span>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ $profile->portfolio_github_folder_link ?? '#' }}" target="_blank" class="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-700/50 bg-slate-800/50 hover:bg-slate-700/50 transition-all group">
